...
// Ordenar comentaris de cada fitxa
function ordenarComentaris(tipus) {
    var comentaris = document.querySelectorAll('.fitxa-comentaris');  
    var comentarisArray = Array.from(comentaris);

    // Gestionar actiu
    var botoData = document.getElementById('boto-ordenar-data');
    var botoVots = document.getElementById('boto-ordenar-vots');
    botoData.classList.remove('active');
    botoVots.classList.remove('active');
    if (tipus === 'data') {
        botoData.classList.add('active');
    } else if (tipus === 'vots') {
        botoVots.classList.add('active');
    }

    console.log("Ordenant per: " + tipus);

    comentarisArray.sort(function(a, b) {   
        if (tipus === 'data') {
            var dataA = new Date(a.getAttribute('data-data'));
            var dataB = new Date(b.getAttribute('data-data'));
            return dataA - dataB; // més antics primer
        } else if (tipus === 'vots') {
            var votsA = parseInt(a.getAttribute('data-vots'));
            var votsB = parseInt(b.getAttribute('data-vots'));
            return votsB - votsA; // millor valorats primer
        }       
    });

    var contenedorComentaris = document.getElementById('div-comentaris');   
    comentarisArray.forEach(function(comentari) {
        contenedorComentaris.appendChild(comentari); // Escriure els comentaris en el nou ordre
    });
