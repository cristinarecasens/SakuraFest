...
try {
    ...
    if (isset($_GET['id'])) {
        ...
        $stmt_comentaris = $pdo->prepare("SELECT c.*, u.nom AS nom_usuari FROM comentaris c JOIN usuaris u ON c.user_id = u.id WHERE c.activitat_id = ? ORDER BY c.data_hora DESC");
...
<body>
    ...
    <main>
        ...
        <div id="div-comentaris">
            ...
            <?php if (count($comentaris) > 0): ?>
            <div class="nav-ordenar-comentaris">
                <a href="#" onclick="ordenarComentaris('data')" class="boto-ordenar active" id="boto-ordenar-data">Més recents</a>
                <a href="#" onclick="ordenarComentaris('vots')" class="boto-ordenar" id="boto-ordenar-vots">Millor valorats</a>       
            </div>
            <?php endif; ?>

            <?php if (count($comentaris) === 0): ?>
                <p style="text-align:center; color:#222; font-weight:600; margin-top:24px;">No hi ha comentaris a mostrar.</p>
            <?php endif; ?>

            <?php foreach ($comentaris as $comentari): ?>
                <div class="fitxa-comentaris" style="position:relative;" data-data="<?= htmlspecialchars($comentari['data_hora']) ?>" data-vots="<?= intval($comentari['positiu']) - intval($comentari['negatiu']) ?>">
